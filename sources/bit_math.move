...
public fun closest_bit_right(x: u256, bit: u8): (u8, bool) {
    ...
    return if (x == 0) {
        ...
        (most_significant_bit(x) - shift, true)
    }
}
...
public fun closest_bit_left(x: u256, bit: u8): (u8, bool) {
    ...
    return if (x == 0) {
        ...
        (least_significant_bit(x) + bit, true)
    }
}
 ...
 */
public fun most_significant_bit(mut bits: u256): u8 {
    let mut msb = 0;
    if (bits > U128_MAX) {
        bits = bits >> 128;
        msb = 128;
 ...
 */
public fun least_significant_bit(mut bits: u256): u8 {
    let mut lsb = 0;
